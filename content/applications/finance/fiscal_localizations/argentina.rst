...
The liquidity product direct sale is used in cases where sales are made to a third party. In these
cases, the seller and the proprietary company of the goods can register their corresponding sales
and purchase. This feature is included when :ref:`installing <general/install>` the *Argentinian
Electronic Invoicing* module (`l10n_ar_edi`).
...
.. important::
   Note that by default these document types are *inactive*. For the workflow to work, it is
   necessary to *activate* these documents.

To activate them, navigate to :menuselection:`Accounting --> Configuration --> Document Types` and
remove any active filters. Then, find the documents and activate them by toggling the option
:guilabel:`Active`.
...
Product*. This journal needs to have an active synchronization with the AFIP as this electronic
...
- :guilabel:`Use Documents`: check this field for the journal to select the electronic document type
  (in this case *sales account* and *liquid product*).
- :guilabel:`Is AFIP POS`: select this field for the journal to be able to generate electronic
  documents (in this case *sales account* and *liquid product*).
...
- :guilabel:`Type`: select :guilabel:`Sales` from the drop-down menu for sale invoice journals.
